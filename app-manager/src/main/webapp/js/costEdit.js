Ext.onReady(function () {
    ...
    this.save = function () {
        ...
        if (!regexVerify('intege1', getById("startMileage"))) {
            alert("日末里程数请填写正确的数字！");
            document.getElementById("startMileage").focus();
            return false;
        }
        if (!regexVerify('intege1', getById("endMileage"))) {
            alert("日末里程数请填写正确的数字！");
            document.getElementById("endMileage").focus();
            return false;
        }
        if (!regexVerify('intege1', getById("workload"))) {
            alert("作业量请填写正确的数字！");
            document.getElementById("workload").focus();
            return false;
        }
        if (!regexVerify('intege1', getById("fuel"))) {
            alert("加油量请填写正确的数字！");
            document.getElementById("fuel").focus();
            return false;
        }

        if (!regexVerify('tingke', getById("fuelAmount"))) {
            alert("加油金额请填写正确的金额！");
            document.getElementById("fuelAmount").focus();
            return false;
        }
        if (!regexVerify('tingke', getById("baoyangAmount"))) {
            alert("保养费用请填写正确的金额！");
            document.getElementById("baoyangAmount").focus();
            return false;
        }
        if (!regexVerify('tingke', getById("fixAmount"))) {
            alert("维修费用请填写正确的金额！");
            document.getElementById("fixAmount").focus();
            return false;
        }

        if (!regexVerify('intege1', getById("shiguTimes"))) {
            alert("事故次数请填写正确的数字！");
            document.getElementById("shiguTimes").focus();
            return false;
        }

        if (!regexVerify('tingke', getById("shiguAmount"))) {
            alert("事故金额请填写正确的金额！");
            document.getElementById("shiguAmount").focus();
            return false;
        }
        if (!regexVerify('tingke', getById("shiguOutAmount"))) {
            alert("事故保险外赔偿金额请填写正确的金额！");
            document.getElementById("shiguOutAmount").focus();
            return false;
        }
        if (!regexVerify('tingke', getById("baoxianAmount"))) {
            alert("保险费用请填写正确的金额！");
            document.getElementById("baoxianAmount").focus();
            return false;
        }
        if (!regexVerify('tingke', getById("yearCheckAmount"))) {
            alert("年检费用请填写正确的金额！");
